  ...
  return (
    <div style={{ padding: '32px 40px', maxWidth: 1300, margin: '0 auto' }}>
      <div style={{ marginBottom: 24 }}>
        <h1 style={{ fontSize: 24, fontWeight: 800, color: '#10233F', margin: 0, letterSpacing: '-0.02em' }}>Audit Log</h1>
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 20, flexWrap: 'wrap' }}>
        <input
          style={{ padding: '9px 14px', border: '1px solid #DCE8F7', borderRadius: 8, fontSize: 13, color: '#10233F', background: '#fff', width: 220 }}
          placeholder="Search provider or user…"
          value={search}
          onChange={e => handleFilterChange('search', e.target.value)}
        />
        <select
          style={{ padding: '9px 14px', border: '1px solid #DCE8F7', borderRadius: 8, fontSize: 13, color: '#10233F', background: '#fff' }}
          value={actionFilter}
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {logs.map((log, i) => (
                <tr
                  key={log.id}
                  style={{ borderTop: '1px solid #EAF1FA', background: i % 2 === 0 ? '#fff' : '#FAFAFA' }}
                >
                  ...
                  <td style={{ padding: '12px 16px' }}>
                    {log.providerName ? (
                      <div>
                        <div style={{ fontSize: 13, fontWeight: 600, color: '#10233F' }}>{log.providerName}</div>
                  ...
                  <td style={{ padding: '12px 16px' }}>
                    {log.performedByName ? (
                      <div>
                        <div style={{ fontSize: 13, fontWeight: 600, color: '#10233F' }}>{log.performedByName}</div>
      ...
      {totalPages > 1 && (
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, marginTop: 20 }}>
          <button
            ...
            disabled={page === 1}
            style={{ padding: '7px 14px', border: '1px solid #DCE8F7', borderRadius: 8, background: page === 1 ? '#F8FAFC' : '#fff', color: page === 1 ? '#CBD5E1' : '#374151', fontSize: 13, fontWeight: 600, cursor: page === 1 ? 'not-allowed' : 'pointer' }}
          >
            ← Prev
          </button>
          ...
          <button
            ...
            style={{ padding: '7px 14px', border: '1px solid #DCE8F7', borderRadius: 8, background: page === totalPages ? '#F8FAFC' : '#fff', color: page === totalPages ? '#CBD5E1' : '#374151', fontSize: 13, fontWeight: 600, cursor: page === totalPages ? 'not-allowed' : 'pointer' }}
          >
            Next →
          </button>
        </div>
