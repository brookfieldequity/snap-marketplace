...
- Room-Count Cards (Scheduling Tools > Room-Count Cards): before building a month's schedule, the coordinator sends each site a tokenized no-login link asking how many rooms run each day. Store one or two contacts per site once, then it is one click to send (a checkbox picks which sites to send to, or send to all). Returned counts become the source of truth in the Schedule Builder — they override the coverage-template default for that month. The site can also leave a per-day note (e.g. "one all-day spine case"), which appears as a post-it on that date in the Schedule Builder calendar.
